...
Процедура ЗаполнитьКоллекциюОбъектовМД()
	...
	ЗаполнитьКоллекциюОбъектовМД_ОсновныеОбъекты();
...
Процедура ЗаполнитьКоллекциюОбъектовМД_ОсновныеОбъекты()
	...
	Если НРег(ПолучитьРасширениеФайла(КопияФайлаСоСпискомОбъектов)) = ".xml" Тогда
		ЗаполнитьКоллекциюОбъектовМД_ОсновныеОбъекты_xml(КопияФайлаСоСпискомОбъектов);
	Иначе
		ЗаполнитьКоллекциюОбъектовМД_ОсновныеОбъекты_ИзТабличногоДокумента(КопияФайлаСоСпискомОбъектов);
	КонецЕсли;
	
КонецПроцедуры
Процедура ЗаполнитьКоллекциюОбъектовМД_ОсновныеОбъекты_ИзТабличногоДокумента(ПутьКФайлу)
	...
	Для сч = 2 по СписокЗагружаемыхОбъектов.ВысотаТаблицы Цикл
		...
		ОбъектМД_ДобавитьВКоллекцию(ТипОбъекта, ВидОбъекта, Ложь);
		
	КонецЦикла;
	
КонецПроцедуры
Процедура ЗаполнитьКоллекциюОбъектовМД_ОсновныеОбъекты_xml(ПутьКФайлу)
	...
	Для Каждого Узел Из НайденныеУзлы Цикл
		...
		ОбъектМД_ДобавитьВКоллекцию(Тип, Вид, Ложь);
...
Процедура ЗаполнитьКоллекциюОбъектовМД_ЗаглушкиЗаглушек()
	...
	ЗаполнитьКоллекциюОбъектовМД_ЗаглушкиЗаглушек_ПланыСчетов();
	ЗаполнитьКоллекциюОбъектовМД_ЗаглушкиЗаглушек_РегистрыСРегистраторами();
	ЗаполнитьКоллекциюОбъектовМД_ЗаглушкиЗаглушек_Стили();
	
КонецПроцедуры
Процедура ЗаполнитьКоллекциюОбъектовМД_ЗаглушкиЗаглушек_ПланыСчетов()
...
КонецФункции
Процедура ЗаполнитьКоллекциюОбъектовМД_ЗаглушкиЗаглушек_Стили()
	Состояние("Заполнение заглушек стилей");
	
	МассивЗаглушек = Новый Массив;
	
	Отбор = Новый Структура;
	Отбор.Вставить("ЭтоЗаглушка", Истина);
	Отбор.Вставить("Тип","Style");
	НайденныеСтроки = КоллекцияОбъектовМД.НайтиСтроки(Отбор);
	Для Каждого Строка Из НайденныеСтроки Цикл
		
		НайденныеФайлы = ОбъектМД_НайтиФайлОписанияИзКаталога_Ext(Строка, КаталогИсходныхФайлов, "Style");
		Для каждого НайденныйФайл из НайденныеФайлы Цикл
...
Процедура СкопироватьФайлыОбъектовМД()
	...
	Для каждого Строка из КоллекцияОбъектовМД Цикл
		...
		ОбъектМД_СкопироватьФайлы(Строка);
...
Процедура ПочиститьСсылкиВЗаглушках()
	...
	ШаблоныУдаления = ПочиститьСсылкиВЗаглушках_ШаблоныУдаления();
	ШаблоныОчистки = ПочиститьСсылкиВЗаглушках_ШаблоныОчистки();
...
КонецПроцедуры
Функция ПочиститьСсылкиВЗаглушках_ШаблоныУдаления()
	
	Массив = Новый Массив;
	Массив.Добавить("Template");
	Массив.Добавить("v8:Type");
	Массив.Добавить("Page");
	Массив.Добавить("object");
	Массив.Добавить("Group");
	
	Возврат Массив;
	
КонецФункции
Функция ПочиститьСсылкиВЗаглушках_ШаблоныОчистки()
	Массив = Новый Массив();
	Массив.Добавить("InputByString");
	Массив.Добавить("Type");
	Массив.Добавить("DefaultObjectForm");
	Массив.Добавить("DefaultFolderForm");
	Массив.Добавить("DefaultListForm");
	Массив.Добавить("DefaultChoiceForm");
	Массив.Добавить("DefaultFolderChoiceForm");
	Массив.Добавить("AuxiliaryObjectForm");
	Массив.Добавить("AuxiliaryFolderForm");
	Массив.Добавить("AuxiliaryListForm");
	Массив.Добавить("AuxiliaryChoiceForm");
	Массив.Добавить("AuxiliaryFolderChoiceForm");
	Массив.Добавить("ChoiceForm");
	Массив.Добавить("BasedOn");
	Массив.Добавить("RegisterRecords");
	Массив.Добавить("CharacteristicExtValues");
	Массив.Добавить("Owners");
	Массив.Добавить("DefaultSaveForm");
	Массив.Добавить("DefaultLoadForm");	
	Массив.Добавить("DefaultForm");	
	Массив.Добавить("MainDataCompositionSchema");	
	Массив.Добавить("DefaultSettingsForm");	
	Массив.Добавить("AuxiliaryForm");	
	Массив.Добавить("DefaultVariantForm");	
	Массив.Добавить("ChoiceParameters");	
	Массив.Добавить("FillValue");	
	Массив.Добавить("Numerator");	
	Массив.Добавить("DefaultRecordForm");	
	Массив.Добавить("AuxiliaryRecordForm");	
	Массив.Добавить("Addressing");	
	Массив.Добавить("AddressingDimension");	
	Массив.Добавить("CurrentPerformer");	
...
Процедура ОбъектМД_СкопироватьФайлы(СтрокаОбъекта)
	
	Если мЭтоИерархическаяВыгрузка Тогда
		ОбъектМД_СкопироватьФайлы_СИерархией(СтрокаОбъекта);
	Иначе
		ОбъектМД_СкопироватьФайлы_Линейно(СтрокаОбъекта);
	КонецЕсли;
	
КонецПроцедуры
Процедура ОбъектМД_СкопироватьФайлы_Линейно(СтрокаОбъекта)
...
КонецПроцедуры
Процедура ОбъектМД_СкопироватьФайлы_СИерархией(СтрокаОбъекта)
	
	ОбъектМД_СкопироватьФайлы_СИерархией_СоздатьВложенныеКаталоги(СтрокаОбъекта);
	
	ОбъектМД_СкопироватьФайлы_СИерархией_СкопироватьФайлы(СтрокаОбъекта);
...
КонецПроцедуры
Процедура ОбъектМД_СкопироватьФайлы_СИерархией_СкопироватьФайлы(СтрокаОбъекта)
